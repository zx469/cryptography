...
**答案：** _6c73d5240a948c86981bc284814d__________
...
**答案：Adv PRG​ [A,G]= ​ Pr k← R K [A(G(k))=1]−Pr r← R​ {0,1} n ​ [A(r)=1] ​
**
...
**答案：接近 1：统计测试 A 能以极高概率区分 PRG 输出的伪随机串和真随机串，说明 G 不是安全的 PRG；
接近 0：统计测试 A 无法区分伪随机串和真随机串（区分概率接近随机猜测），说明 G 是安全的 PRG。**
...
**答案：Pr[A(G(k))=1]=0，Pr[A(r)=1]=0，因此优势 = ∣0−0∣=0。**
...
PRG
​
 [A,G]=∣2/3−1/2∣=∣4/6−3/6∣=1/6≈0.1667
。**
...
**答案：不安全。安全的 PRG 要求对于任意高效统计测试 A，
Adv 
PRG
​
 [A,G]
 是可忽略的（接近 0）。本题中优势为 1/6（约 16.67%），显著大于可忽略阈值，因此 G 不是安全的 PRG。**
...
**答案：完美保密性定义：对于任意明文m∈M、密文c∈C，有Pr k[E(k,m)=c]=1/∣K∣（即每个密文由等概率的密钥生成）；且对于任意m 0 ,m 1 ∈M、c∈C，有Pr[E(k,m 0 )=c]=Pr[E(k,m 1 )=c]。**
...
**答案：OTP 的加密公式为E(k,m)=m⊕k=c，变形得k=m⊕c。对于任意固定的m和c，异或运算的唯一性保证了恰好存在 1 个密钥、k满足该等式。**
...
由 (b)，任意
m,c
恰好存在 1 个密钥k使得E(k,m)=c，因此Pr k [E(k,m)=c]=1/N
（密钥均匀随机选择）；
对于任意m 0 ,m 1 ∈M、c∈C，均有Pr[E(k,m 0 )=c]=1/N=Pr[E(k,m 1)=c]，满足完美保密性的定义；
综上，OTP 具有完美保密性。**
...
**答案：语义安全的定义：对于任意高效攻击者A、任意等长明文m 0 ,m 1，有Adv SS [A,E]=∣Pr[A(c)=1∣c=E(k,m 0 )]−Pr[A(c)=1∣c=E(k,m 1 )]∣≤negl(n)（可忽略量）。已知存在算法A
能从密文c推断LSB(m)，构造攻击者B：选择m 0（LSB(m 0 )=0）和m 1（LSB(m 1 )=1
）；收到密文c后，用A计算LSB(m)，若为 0 则猜测c=E(k,m 0​ )，否则猜测c=E(k,m 1​)；此时猜中，优势∣1−0.5∣=0.5（非可忽略），因此E不是语义安全的。**
...
**答案：选择消息：m 0为任意LSB(m 0​)=0的明文，m 1为任意LSB(m 1 )=1的等长明文（例如m 0 =0 n，m 1 =1 n）；猜测策略：收到密文c后，调用子程序A(c)得到LSB(m)：若LSB(m)=0，则猜测b=0（密文对应m 0）；若LSB(m)=1，则猜测b=1（密文对应m 1）。**
...
**答案：语义安全优势公式：Adv SS​ [B,E]=∣Pr[B(c)=1∣c=E(k,m 1​ )]−Pr[B(c)=1∣c=E(k,m 0​ )]∣。Pr[B(c)=1∣c=E(k,m 1 )]=1（A能正确推断LSB(m 1​)=1，B猜b=1）；Pr[B(c)=1∣c=E(k,m 0)]=0（A能正确推断LSB(m 0)=0，B猜b=0）；因此优势 = ∣1−0∣=1。**
...
**答案：设Y=m 0​ （固定明文），X=k（均匀随机变量，k← R​ {0,1} n ）。根据异或的均匀性定理：若X是独立的均匀随机变量，则Z=Y⊕X是均匀分布的。因此c=k⊕m 0 在{0,1} n 上均匀分布。**
...
**答案：同理，设Y=m 1 （固定明文），X=k（均匀随机变量）。由异或的均匀性定理，Z=Y⊕X=k⊕m 1​  在{0,1} n 上均匀分布。**
...
**答案：由 (c)，k⊕m 0和k⊕m 1的分布完全相同，记为D；因此Pr[A(k⊕m 0 )=1]=Pr c←D [A(c)=1]，Pr[A(k⊕m 1​ )=1]=Pr c←D [A(c)=1]；两者的差值为 0，即Adv SS [A,OTP]=∣0∣=0；综上，OTP 满足语义安全的定义（优势为 0）。**
