...
**答案：** ___6c73d5240a948c86981bc2808548________
...
**答案：**\text{Adv}_{\text{PRG}}[A, G] = \left| \Pr_{k \xleftarrow{R} K}[A(G(k)) = 1] - \Pr_{r \xleftarrow{R} \{0,1\}^n}[A(r) = 1] \right|
...
**答案：**\text{Adv}_{\text{PRG}}[A, G] 接近1：统计测试A可以非常容易地区分PRG的输出和真随机串，说明该PRG对A是不安全的。

- \text{Adv}_{\text{PRG}}[A, G] 接近0：统计测试A无法有效区分PRG的输出和真随机串，说明该PRG对A是安全的。
...
**答案：**对于任意明文 m, m' \in M 和任意密文 c \in C，都满足：
\Pr[E(k, m) = c] = \Pr[E(k, m') = c]
其中密钥 k 是从密钥空间 K 中均匀随机选取的。
...
**答案：**对于OTP加密算法 E(k, m) = m \oplus k，对于任意固定的 m 和 c，满足 m \oplus k = c 的密钥 k 是唯一的，即 k = m \oplus c。
由于 k 是均匀随机的，因此：
\Pr[E(k, m) = c] = \Pr[k = m \oplus c] = \dfrac{1}{|K|} = \dfrac{1}{2^n}
这个概率与明文 m 无关，因此对于任意 m'，都有 \Pr[E(k, m') = c] = \dfrac{1}{2^n}
...
**答案：**由(a)和(b)可知，对于任意明文 m 和密文 c，\Pr[E(k, m) = c] 是一个与 m 无关的常数。根据贝叶斯定理：
\Pr[M = m | C = c] = \dfrac{\Pr[C = c | M = m] \cdot \Pr[M = m]}{\Pr[C = c]}
其中 \Pr[C = c] = \sum_{m'} \Pr[C = c | M = m'] \cdot \Pr[M = m'] = \dfrac{1}{2^n} \sum_{m'} \Pr[M = m'] = \dfrac{1}{2^n}。
代入得：
\Pr[M = m | C = c] = \dfrac{\dfrac{1}{2^n} \cdot \Pr[M = m]}{\dfrac{1}{2^n}} = \Pr[M = m]
...
**答案：**语义安全性的定义是：对于任意高效攻击者 B，其优势
\text{Adv}_{\text{SS}}[B, E] = \left| \Pr[B \text{ outputs 1 in EXP(0)}] - \Pr[B \text{ outputs 1 in EXP(1)}] \right|
是可忽略的。
现在构造攻击者 B：
 
1. B 选择两个等长明文 m_0 和 m_1，其中 \text{LSB}(m_0) = 0，\text{LSB}(m_1) = 1。

2. 挑战者加密 m_b 得到密文 c 并发送给 B。

3. B 调用算法 A 对 c 进行计算，得到 l = A(c) = \text{LSB}(m_b)。

4. 如果 l = 1，B 输出1；否则输出0。
 
此时：
 
- 在EXP(0)中，m_b = m_0，\text{LSB}(m_0) = 0，因此 A(c) = 0，B 输出0，即 \Pr[B \text{ outputs 1 in EXP(0)}] = 0。

- 在EXP(1)中，m_b = m_1，\text{LSB}(m_1) = 1，因此 A(c) = 1，B 输出1，即 \Pr[B \text{ outputs 1 in EXP(1)}] = 1。
 
因此 \text{Adv}_{\text{SS}}[B, E] = |0 - 1| = 1，这是一个不可忽略的优势，所以加密方案 E 不是语义安全的。
...
**答案：**选择的两条消息：m_0（最低有效位为0）和 m_1（最低有效位为1）。

- 收到密文 c 后，B 调用算法 A 得到 \text{LSB}(m_b)。如果结果为1，则猜测 b=1；否则猜测 b=0。
...
**答案：**在EXP(0)中，密文 c = k \oplus m_0，其中 k 是从 \{0,1\}^n 中均匀随机选取的。
根据异或的性质：任何随机变量与独立的均匀随机变量异或后，结果仍是均匀分布。
因此对于任意固定的 m_0，c = k \oplus m_0 在 \{0,1\}^n 上是均匀分布的，即对于任意 c \in \{0,1\}^n，\Pr[c = k \oplus m_0] = \dfrac{1}{2^n}
...
**答案：**与(a)同理，在EXP(1)中，密文 c = k \oplus m_1，其中 k 是均匀随机的。因此 c 也在 \{0,1\}^n 上均匀分布，即对于任意 c \in \{0,1\}^n，\Pr[c = k \oplus m_1] = \dfrac{1}{2^n}。
...
**答案：**由(a)和(b)可知，EXP(0)和EXP(1)中的密文 c 都服从 \{0,1\}^n 上的均匀分布，因此它们的分布是完全相同的
...
**答案：**由于两个实验中的密文分布完全相同，因此对于任意攻击者 A，其在两个实验中输出1的概率是相等的，即：
\Pr[A(k \oplus m_0) = 1] = \Pr[A(k \oplus m_1) = 1]
因此语义安全优势：
\text{Adv}_{\text{SS}}[A, \text{OTP}] = \left| \Pr[A(k \oplus m_0) = 1] - \Pr[A(k \oplus m_1) = 1] \right| = 0
对于任意攻击者 A 都成立，因此OTP是语义安全的
