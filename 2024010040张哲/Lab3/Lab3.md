...
$\mathrm{Adv}_{\mathrm{PRG}}[A, G]$ 接近 1：说明区分器 A 能以极高概率区分 G 输出的伪随机串和真随机串，意味着 G 不是安全的伪随机数生成器（PRG），其输出的伪随机性被攻破。
$\mathrm{Adv}_{\mathrm{PRG}}[A, G]$ 接近 0：说明区分器 A 无法有效区分伪随机串和真随机串，G 输出的串在统计上与真随机串不可区分，G 是安全的 PRG。
...
```
该 PRG 不安全。原因：安全的 PRG 要求对任意多项式时间的统计测试（区分器）A，其优势 AdvPRG​[A,G] 是可忽略的（negligible）。本题中构造的区分器 A 得到的优势为 61​，是一个不可忽略的常数，说明该区分器能以显著概率区分 PRG 输出与真随机串，因此该 PRG 不满足安全要求。
```
...
```
语义安全的核心要求是：不存在多项式时间攻击者，能以显著高于 1/2 的概率，从密文推断出明文的任意非平凡信息。
已知存在高效算法A，总能从密文c中准确推断出明文m的最低有效位LSB(m)，这说明该加密方案会泄露明文的非平凡信息（最低位），违反了语义安全的定义，因此该加密方案E=(E,D)不是语义安全的。
```
